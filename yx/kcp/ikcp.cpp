...
void ikcp_flush(ikcpcb *kcp)
{
	...
	seg.frg = 0;
	seg.wnd = ikcp_wnd_unused(kcp);  // 远端主机可以接收的最大序列号
	seg.una = kcp->rcv_nxt;
	seg.len = 0;
	seg.sn = 0;
	seg.ts = 0;
	...
	while (_itimediff(kcp->snd_nxt, kcp->snd_una + cwnd) < 0) {
		...
		newseg->wnd = seg.wnd;
		newseg->ts = current;
		newseg->sn = kcp->snd_nxt++;    //发送序列递增，snd_buf也是按序列递增存放。
		newseg->una = kcp->rcv_nxt;
		newseg->resendts = current;     //从snd_queue移到snd_buf中时，需立即发送。
		newseg->rto = kcp->rx_rto;
		newseg->fastack = 0;
		newseg->xmit = 0;               //xmit为发送的个数，即记录本seg被发送多次。
	}
...
//Modify Begin
//by ZC. 2016-12-28 15:10.
void ikcp_flush_snd_buf(ikcpcb *kcp) {
  IUINT32 current = kcp->current;
  char *buffer = kcp->buffer;
  char *ptr = buffer;
  IUINT32 wnd_unused = ikcp_wnd_unused(kcp);  // 远端主机可以接收的最大序列号
  int size;
  IUINT32 resent, cwnd;
  IUINT32 rtomin;
  struct IQUEUEHEAD *p;
...
//Modify Begin
//by ZC. 2016-12-27 10:12.
IUINT32 ikcp_overhead() {
