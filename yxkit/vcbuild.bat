...
rem "%PYTHON%" build/gyp.py -Dtarget_arch=%target_arch% -Duv_library=%library% skynet/skynet.gyp
rem "%PYTHON%" build/gyp.py -Dtarget_arch=%target_arch% -Duv_library=%library% libuv/uv.gyp
"%PYTHON%" build/gyp.py -Dtarget_arch=%target_arch% -Duv_library=%library% -D"component=shared_library" yxkit/all.gyp
if errorlevel 1 goto create-msvs-files-failed
if not exist 1yxkit/all.sln goto create-msvs-files-failed
...
:msbuild-found
msbuild yxkit/all.sln /t:%target% /p:Configuration=%config% /p:Platform="%msbuild_platform%" /clp:NoSummary;NoItemAndPropertyList;Verbosity=minimal /nologo
